...
def outer():
    ...
    def calculate_sum_root_to_leaf(node, node_num):
        ...
        if is_leaf(node):
            ...
            node_num = node_num * 10 + node.val
            current_sum = node_num
            ...
            if node.left:
                # left(node, result)
                calculate_sum_root_to_leaf(node.left, node_num)

            if node.right:
                calculate_sum_root_to_leaf(node.right, node_num)
                # right(node, result)
